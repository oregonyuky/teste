...
{   private Conexao con;
    ...
    public ArrayList<Musica> pesquisar(String termo) {
        GeneroDAL gdal = new GeneroDAL(context);
        ArrayList<Musica> musicas = new ArrayList<>();
        String valor = "%" + termo.trim() + "%";
        Cursor cursor = con.consultar(
                "SELECT * FROM musica WHERE mus_titulo LIKE ? COLLATE NOCASE " +
                        "OR mus_interprete LIKE ? COLLATE NOCASE ORDER BY mus_titulo",
                new String[]{valor, valor});
        while (cursor.moveToNext()) {
            musicas.add(new Musica(cursor.getInt(0), cursor.getInt(1),
                    cursor.getString(2), cursor.getString(3),
                    gdal.get(cursor.getInt(4)), cursor.getDouble(5)));
        }
        cursor.close();
